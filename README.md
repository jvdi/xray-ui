...
1. 首先从 https://github.com/jvdi/xray-ui/releases 下载最新的压缩包，一般选择 `amd64`架构
...
xray-ui支持通过Tg机器人实现每日流量通知，面板登录提醒等功能，使用Tg机器人，需要自行申请
...
首先在安装了 v2-ui 的服务器上安装最新版 xray-ui，然后使用以下命令进行迁移，将迁移本机 v2-ui 的 `所有 inbound 账号数据`至 xray-ui，`面板设置和用户名密码不会迁移`

> 迁移成功后请 `关闭 v2-ui`并且 `重启 xray-ui`，否则 v2-ui 的 inbound 会与 xray-ui 的 inbound 会产生 `端口冲突`
...
[![Stargazers over time](https://starchart.cc/jvdi/xray-ui.svg)](https://starchart.cc/jvdi/xray-ui)
